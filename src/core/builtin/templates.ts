/**
 * The offline builder's project templates.
 *
 * These are not snippets — each one is a complete, playable/usable app in plain
 * HTML, CSS and JS with no build step and no dependencies, so they run in the
 * Preview panel on any device including a phone. This is the floor: when there
 * is no network, no account and no model, typing an idea still produces
 * something real.
 */
...
const PALETTES: Record<string, { bg: string; panel: string; ink: string; dim: string; accent: string; accent2: string }> = {
  midnight: { bg: '#0d1017', panel: '#161b26', ink: '#e9edf6', dim: '#98a1b6', accent: '#7c8cff', accent2: '#48d8e6' },
  ember: { bg: '#17100e', panel: '#241a16', ink: '#f6ece7', dim: '#bda79c', accent: '#ff7a45', accent2: '#ffc857' },
  grove: { bg: '#0c1512', panel: '#14231e', ink: '#e7f4ee', dim: '#93b3a5', accent: '#4ade80', accent2: '#22d3ee' },
  orchid: { bg: '#140f1b', panel: '#1f1729', ink: '#f0eaf7', dim: '#b0a1c4', accent: '#b98cff', accent2: '#ff7bb0' },
};
...
function paletteFor(seed: string) {
  ...
  return PALETTES[keys[total % keys.length]];
}

function baseCss(seed: string, extra: string): string {
...
function page(title: string, body: string, opts: { script?: boolean } = {}): string {
  return `<!doctype html>
<html lang="en">
  <head>
    <meta charset="utf-8" />
  ...
  <body>
${body}
${opts.script === false ? '' : '    <script src="app.js"></script>'}
  </body>
</html>
`;
}
...
function maze(prompt: string): Blueprint {
  return {
    key: 'maze',
    title: 'Maze',
    plan: ['Generate a random maze', 'Draw it on a canvas', 'Move with the arrow keys or swipe', 'Detect reaching the exit'],
    summary:
      'A maze game. Every round carves a brand-new maze with a depth-first walk, and you steer the dot to the green exit with the arrow keys, WASD, or a swipe on a phone.',
    files: {
      'index.html': page(
        'Maze',
        `    <h1>Maze</h1>
    ...
    <button id="again">New maze</button>`,
      ),
      'style.css': baseCss(
        prompt,
        `canvas {
...
function snake(prompt: string): Blueprint {
  return {
    key: 'snake',
    title: 'Snake',
    plan: ['Draw the grid', 'Move the snake on a tick', 'Grow when it eats', 'End on a wall or itself'],
    summary:
      'Snake. Arrow keys or swipe to steer, the tail grows each time you eat, and the speed creeps up as your score climbs. Your best score is kept in the browser.',
    files: {
      'index.html': page(
        'Snake',
        `    <h1>Snake</h1>
    ...
    <button id="again">New game</button>`,
      ...
      'style.css': baseCss(
        prompt,
        `canvas { touch-action: none; max-width: min(92vw, 440px); height: auto; aspect-ratio: 1; }`,
      ),
      'app.js': `// Snake on a 20x20 grid.
var canvas = document.getElementById('board');
var ctx = canvas.getContext('2d');
...
function todo(prompt: string): Blueprint {
  return {
    key: 'todo',
    title: 'List',
    plan: ['Lay out the input and list', 'Add, complete and delete items', 'Save to the browser', 'Show what is left'],
    summary:
      'A list app. Add items, tick them off, delete them, and everything is saved in the browser so it survives a refresh.',
    files: {
      'index.html': page(
        'My List',
        `    <h1>My List</h1>
    <div class="panel card">
      ...
      'style.css': baseCss(
        prompt,
        `.card { width: min(94vw, 460px); padding: 18px; }
...
.foot .ghost { margin-left: auto; padding: 5px 10px; font-size: 13px; border-color: transparent; }`,
      ),
      'app.js': `// A list that remembers itself.
...
function timer(prompt: string): Blueprint {
  return {
    key: 'timer',
    title: 'Timer',
    plan: ['Draw the progress ring', 'Count down and update it', 'Start, pause and reset', 'Chime when it finishes'],
    summary:
      'A focus timer with a circular progress ring. 25 minutes by default, with a short break preset, and it chimes with the Web Audio API when the session ends.',
    files: {
      'index.html': page(
        'Timer',
        `    <h1>Focus</h1>
    ...
    <div class="row">
      ...
      'style.css': baseCss(
        prompt,
        `.dial { position: relative; display: grid; place-items: center; padding: 18px; border-radius: 50%; }
...
.preset { padding: 7px 13px; font-size: 13px; }`,
      ),
      'app.js': `// Focus timer with an SVG progress ring.
...
function landing(prompt: string): Blueprint {
  const subject = prompt
    .replace(/^\s*(make|build|create)\s+(me\s+)?(a|an|the)?\s*/i, '')
    .replace(/\b(landing\s*page|website|site|page|for)\b/gi, ' ')
    .replace(/\s+/g, ' ')
    .trim();
  const name = (subject || 'Northwind').split(' ').slice(0, 3).join(' ');
  const titled = name.charAt(0).toUpperCase() + name.slice(1);

  return {
    key: 'landing',
    title: titled,
    plan: ['Write the hero', 'Add the feature row', 'Style it', 'Make it work on a phone'],
    summary: `A one-page site for ${titled} — hero, three features, and a closing call to action. Responsive, no dependencies, and easy to edit: the copy is all in index.html.`,
    files: {
      'index.html': page(
        titled,
        `    <main>
      <section class="hero">
        <p class="eyebrow">${titled}</p>
      ...
      'style.css': baseCss(
        prompt,
        `body { justify-content: flex-start; padding: 0; display: block; }
...
.closer h2 { margin: 0; font-size: clamp(24px, 4vw, 34px); }`,
      ),
    },
  };
}
...
function starter(prompt: string): Blueprint {
  const clean = prompt.replace(/[<>&]/g, '').slice(0, 160) || 'your idea';
  return {
    key: 'starter',
    title: 'Starter',
    plan: ['Create the page', 'Style it', 'Wire up a little interactivity'],
    summary:
      'A styled starter page with your idea written into it and a working counter, so there is something real on screen to build from. Ask for a specific change and it grows from here.',
    files: {
      'index.html': page(
        'Starter',
        `    <h1>Ready to build</h1>
    <p class="hint">You asked for: <em>${clean}</em></p>
    <div class="panel card">
      <p class="big" id="count">0</p>
      <div class="row"><button id="down">-</button><button id="up">+</button><button id="zero">Reset</button></div>
    </div>
    <p class="hint">Everything here is plain HTML, CSS and JavaScript. Ask for the next change and it gets built on top.</p>`,
      ),
      'style.css': baseCss(
        prompt,
        `.card { padding: 26px 30px; display: grid; gap: 16px; justify-items: center; }
...
const MATCHERS: { test: RegExp; make: (prompt: string) => Blueprint }[] = [
  { test: /\bmaze\b|\blabyrinth\b/i, make: maze },
  { test: /\bsnake\b/i, make: snake },
  { test: /\b(todo|to-do|task list|checklist|shopping list|grocery|notes?|list app|tracker)\b/i, make: todo },
  { test: /\b(timer|pomodoro|countdown|stopwatch|focus)\b/i, make: timer },
  { test: /\b(landing|website|web site|homepage|home page|portfolio|marketing|business site|site for)\b/i, make: landing },
];

export function pickBlueprint(prompt: string): Blueprint {
  for (const matcher of MATCHERS) {
    if (matcher.test.test(prompt)) return matcher.make(prompt);
  }
  return starter(prompt);
}

/** Every archetype the offline builder knows, for the UI to advertise. */
export const ARCHETYPES = ['maze game', 'snake game', 'to-do list', 'focus timer', 'landing page'];
